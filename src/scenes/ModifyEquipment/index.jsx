...
const ModifyEquipment = () => {
  ...
  useEffect(() => {
    const fetchEquipmentData = async () => {
      try {
        const response = await axios.get(`https://nodeapp-4c68.onrender.com/equip/equip/${id}`);
  ...
  const handleModifyEquipment = async (values) => {
    try {
      const response = await axios.put(`https://nodeapp-4c68.onrender.com/equip/equip/${id}`, values);
