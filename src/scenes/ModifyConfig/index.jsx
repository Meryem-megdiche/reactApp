...
const ModifyConfig = () => {
  ...
  useEffect(() => {
    const fetchConfig = async () => {
      try {
        const response = await axios.get(`https://nodeapp-4c68.onrender.com/config/configs/${id}`);
  ...
  const handleModifyConfig = async (values, { setSubmitting }) => {
    try {
      const response = await axios.put(`https://nodeapp-4c68.onrender.com/config/configs/${id}`, values);
