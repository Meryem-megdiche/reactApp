...
const ModifyConfig = () => {
  ...
  useEffect(() => {
    const fetchConfig = async () => {
      try {
        const response = await axios.get(`https://nodeapp-0ome.onrender.com/config/configs/${id}`);
  ...
  const handleModifyConfig = async (values, { setSubmitting }) => {
    try {
      const response = await axios.put(`https://nodeapp-0ome.onrender.com/config/configs/${id}`, values);
