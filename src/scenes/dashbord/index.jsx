...
socket.on('newAlert', (alert) => {
  ...
  const fetchBarChartData = async () => {
    try {
      if (selectedEquipments.length > 0 && startDate && endDate) {
        const response = await axios.post('https://nodeapp-0ome.onrender.com/api/barChartData', {
          startDate: startDate,
          endDate: endDate,
  ...
  const fetchPingCount = async () => {
    try {
      if (selectedEquipments.length > 0 && startDate && endDate) {
        const response = await axios.post('https://nodeapp-0ome.onrender.com/api/pingResults/equip/count', {
          startDate: startDate,
          endDate: endDate,
          equipmentIds: selectedEquipments
        });
...
const fetchInterventionCount = async () => {
  try {
    const response = await axios.post('https://nodeapp-0ome.onrender.com/api/interventions/equip/count', {
      startDate: startDate,
      endDate: endDate,
      equipmentIds: selectedEquipments
    });
...
try {
  if (selectedEquipments.length > 0 && startDate && endDate) {
    const response = await axios.post("https://nodeapp-0ome.onrender.com/api/erij", {
      startDate: startDate,
      endDate: endDate,
      equipmentIds: selectedEquipments
    });
...
const fetchInterventions = async () => {
  try {
    if (selectedEquipments.length > 0 && startDate && endDate) {
      const response = await axios.post('https://nodeapp-0ome.onrender.com/api/interventions/filter', {
        startDate: startDate,
        endDate: endDate,
        equipmentIds: selectedEquipments
      });
...
const fetchResolvedAlertsCount = async () => {
  if (selectedEquipments.length > 0 && startDate && endDate) {
    try {
      const response = await axios.post('https://nodeapp-0ome.onrender.com/api/alerts/resolved/count', {
        startDate: startDate,
        endDate: endDate,
        equipmentIds: selectedEquipments
      });
